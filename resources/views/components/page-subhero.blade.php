    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        <div class="grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-8 items-center">
            <div class="inst-stack-tight">
                ...
                @if ($backHref)
                    <a href="{{ $backHref }}"
                       class="inline-flex min-h-[44px] items-center gap-1.5 text-white/70 hover:text-white text-xs font-semibold uppercase tracking-[0.14em] transition-colors group">
                ...
                {{-- Título h1 --}}
                @if ($title)
                    <h1 class="inst-title text-white drop-shadow-sm">
                        {{ $title }}
                    </h1>
                    <div class="h-[3px] w-16 bg-brand-gold-light"></div>
                @endif

                {{-- Subtítulo --}}
                @if ($subtitle)
                    <p class="max-w-2xl text-white/85 text-sm sm:text-base leading-relaxed">
                        {{ $subtitle }}
                    </p>
                @endif
            </div>
