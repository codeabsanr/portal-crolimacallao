...
@php
    ...
    $routeLabelMap = [
        'institucional.quienes-somos' => 'QUIENES SOMOS',
        'institucional.consejo-directivo' => 'CONSEJO DIRECTIVO',
        'institucional.resena-historica' => 'RESEÑA HISTÓRICA',
        'institucional.simbolos' => 'SÍMBOLOS',
        'institucional.compendio-de-normas' => 'COMPENDIO DE NORMAS',
        'institucional.reglamento' => 'REGLAMENTO',
        'institucional.normatividad' => 'NORMATIVIDAD',
        'institucional.convenios' => 'CONVENIOS',
    ];
    $routeLabel = $routeLabelMap[$routeName] ?? strtoupper(\Illuminate\Support\Str::headline(\Illuminate\Support\Str::afterLast($routeName, '.')));
@endphp

<main id="main">
    <section class="relative overflow-hidden text-white min-h-[320px] md:min-h-[460px] flex items-center">
        <div class="absolute inset-0 bg-cover bg-top" style="background-image: url('{{ asset('assets/img/foto_gestion_2025_2028.jpg') }}');"></div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-14 relative z-10 w-full">
            <div class="grid lg:grid-cols-[1.05fr_0.95fr] gap-5 md:gap-8 items-center">
                <div>
                    ...
                    <p class="mt-2 text-xs font-semibold uppercase tracking-[0.16em] text-brand-gold-light/95">
                        INSTITUCIONAL
                        <span class="text-white/70">/</span>
                        <span class="text-brand-gold-light">{{ $routeLabel }}</span>
                    </p>
                    <p class="inline-block text-[11px] md:text-sm uppercase tracking-[0.18em] bg-primary px-3 py-1 text-white font-bold mt-4 mb-4">{{ $eyebrow }}</p>
                    ...
                    <div class="mt-5 md:mt-6 flex flex-col sm:flex-row gap-2.5 sm:gap-3">
                        ...
                        <a href="{{ route('institucional.normatividad') }}" class="inst-btn-secondary !min-h-0 !py-2.5 !px-4 !text-xs !bg-transparent !border-white/85 !text-white hover:!border-white hover:!bg-white/10 !w-full sm:!w-auto">Marco normativo</a>
                    </div>
                </div>
                <aside class="border border-white/25 bg-white/10 backdrop-blur-sm p-3.5 md:p-5 max-w-xl lg:max-w-none">
                    ...
                    <div class="bg-white/10 border border-white/10 px-3.5 py-3 mt-3">
                        <p class="text-xs uppercase tracking-[0.16em] text-white/80">Decana vigente</p>
                        <p class="text-[1.2rem] sm:text-[1.35rem] md:text-2xl font-black leading-tight text-white mt-1">Obst. Jenny Elenisse Zavaleta Luján</p>
                        <p class="text-sm text-white/85 font-semibold mt-1">Decana Regional</p>
                    </div>
                </aside>
            </div>
        </div>
