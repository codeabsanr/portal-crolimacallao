...
<section class="relative overflow-hidden inst-bg-brand-grid {{ $theme['trace'] }} text-white">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-16">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-12 items-center">
                ...
                <div class="w-full inst-stack-tight">
                    @if ($breadcrumb)
                        <nav aria-label="Breadcrumb principal"
                            class="inline-flex w-fit max-w-full flex-wrap items-center gap-1.5 sm:gap-2 border border-white/25 bg-white/5 px-2.5 sm:px-3 py-2 text-[11px] sm:text-sm font-semibold backdrop-blur-sm">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        @if (!empty($hero['cta1']['text']) && $cta1Href)
                            <a href="{{ $cta1Href }}"
                                class="inline-flex w-full sm:w-auto min-h-[44px] items-center justify-center gap-2 rounded-sm px-6 py-2.5 text-sm sm:text-base font-bold transition-colors shadow-lg hover:shadow-xl {{ $theme['cta1'] }} {{ $routeName === 'contacto' && $cta1IsAnchor ? 'inst-btn-scroll-guide' : '' }}">
                        ...
                        @if (!empty($hero['cta2']['text']) && $cta2Href)
                            <a href="{{ $cta2Href }}"
                                class="inline-flex w-full sm:w-auto min-h-[44px] items-center justify-center rounded-sm border bg-white/5 px-6 py-2.5 text-sm sm:text-base font-bold text-white transition-colors hover:bg-white/10 backdrop-blur-sm {{ $theme['cta2'] }}">
                                {{ $hero['cta2']['text'] }}
                            </a>
                        @endif
                    </div>
            ...
            <aside class="lg:col-span-5 flex justify-center lg:justify-end">
                <div x-data="{
                            ads: @js($ads),
                            current: 0,
                            timer: null,
                    ...
                    <div class="relative inst-hero-ads-stage overflow-hidden border border-white/45 bg-white shadow-[0_24px_50px_-30px_rgba(15,23,42,0.9)] group">
                        <template x-for="(ad, idx) in ads" :key="idx">
                            <article class="absolute inset-0 transition-all duration-500 ease-[cubic-bezier(.2,.7,.2,1)]"
                                ...
                                <div class="absolute top-3 left-3 z-10 drop-shadow-[0_2px_6px_rgba(15,23,42,0.4)]">
                                    <span class="inline-flex px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-white bg-primary"
                                        x-text="ad.tipo"></span>
                                </div>
                            </article>
                        </template>
                    ...
                    <a :href="ads[current].cta_href"
                        class="mt-3 inline-flex w-fit self-center items-center justify-center gap-2 min-h-[44px] px-5 py-3 text-sm font-bold uppercase tracking-[0.1em] text-white transition-[background-color,opacity,box-shadow] duration-200 hover:opacity-90 shadow"
                        :class="{
                    ...
                    <div class="inst-hero-ads-dots mt-3" aria-label="Indicadores de publicidad">
