...
<main id="main" class="md:h-screen md:overflow-hidden flex flex-col md:flex-row">
    <section class="relative flex flex-col w-full md:w-[60%] min-h-[56vh] md:h-screen md:overflow-y-auto bg-background-light border-r border-slate-300/80">
        ...
        <div class="relative z-10 mx-auto flex-1 w-full max-w-4xl px-5 sm:px-6 lg:px-10 py-8 md:py-12">
            <span class="inline-flex items-center rounded-sm border border-slate-300 bg-primary-tint px-3 py-1 text-sm font-semibold uppercase tracking-[0.14em] text-text-main">Consulta publica</span>
            ...
            <p class="mt-4 max-w-2xl text-base md:text-lg text-text-main">Ingrese el numero de colegiatura (COP) o DNI para validar el estado de habilitacion en tiempo real.</p>
            ...
            <form id="consulta-form" class="mt-8 w-full rounded-md border border-primary/45 bg-white p-2">
                <div class="flex flex-col sm:flex-row sm:items-center">
                    ...
                    <div class="flex items-center gap-3 px-3 sm:px-4 py-3 sm:py-0 w-full">
                        ...
                        <input id="consulta-publica" type="text" class="w-full border-none bg-transparent text-xl sm:text-2xl font-medium tracking-wide text-secondary placeholder:text-text-main focus:ring-0" placeholder="Buscar por COP o DNI...">
                    </div>
            ...
            <section id="consulta-resultado" class="hidden mt-8 space-y-4">
                <div class="flex items-center justify-between gap-3">
                    ...
                    <span id="resultado-cop-badge" class="text-xs uppercase tracking-[0.14em] text-text-main font-bold">COP 12874</span>
                </div>
                ...
                <article class="inst-card p-4 md:p-6 border border-primary/25">
                    <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-4 mb-5">
                        <p class="text-sm text-text-main">Estado de habilitacion en tiempo real</p>
                    ...
                    <div class="mt-5 grid sm:grid-cols-3 gap-3">
                        <div class="border border-slate-200 bg-white p-4">
                            <p class="text-xs uppercase tracking-[0.12em] text-text-main font-bold">Carrera</p>
                            <p id="resultado-carrera" class="mt-1 text-2xl font-black text-text-main">Obstetricia</p>
                        </div>
                        <div class="border border-slate-200 bg-white p-4">
                            <p class="text-xs uppercase tracking-[0.12em] text-text-main font-bold">COP</p>
                            <p id="resultado-cop" class="mt-1 text-2xl font-black text-text-main">12874</p>
                        </div>
                        <div class="border border-slate-200 bg-white p-4">
                            <p class="text-xs uppercase tracking-[0.12em] text-text-main font-bold">Especialidad</p>
                            <p id="resultado-especialidad" class="mt-1 text-xl font-black text-text-main">Salud Materno Perinatal</p>
                        </div>
                    </div>

                    <div class="mt-4 grid sm:grid-cols-2 gap-3">
                        <div class="border border-slate-200 bg-white p-4">
                            <p class="text-xs uppercase tracking-[0.12em] text-text-main font-bold">Situacion</p>
                            <p id="resultado-situacion-resumen" class="mt-1 text-2xl font-black text-text-main">Activo</p>
                        </div>
                        <div class="border border-slate-200 bg-white p-4">
                            <p class="text-xs uppercase tracking-[0.12em] text-text-main font-bold">Habilitacion</p>
                    ...
                    <div class="mt-4 grid sm:grid-cols-2 gap-3">
                        <div class="border border-slate-200 bg-white p-4">
                            <p class="text-sm text-text-main">Cursos de especializacion</p>
                            <p id="resultado-curso" class="text-2xl font-black text-text-main mt-1">Emergencias Obstetricas 2024</p>
                        </div>
                        <div class="border border-slate-200 bg-white p-4">
                            <p class="text-sm text-text-main">Ponencias realizadas</p>
                            <p id="resultado-ponencia" class="text-2xl font-black text-text-main mt-1">Lactancia y puerperio seguro</p>
                        </div>
                    </div>
                </article>
            </section>

            <div class="mt-10 flex flex-wrap items-center gap-x-8 gap-y-3 text-sm md:text-base text-text-main">
        ...
        <div class="relative z-10 mx-auto flex h-full w-full max-w-xl flex-col justify-center px-6 sm:px-8 lg:px-10 py-8 lg:py-12">
            ...
            <form method="POST" action="{{ route('login') }}" class="mt-8 space-y-4">
                ...
                <div>
                    ...
                    <div class="relative">
                        <span class="material-icons-outlined pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-text-main">badge</span>
                        <input id="email" name="email" type="text" value="{{ old('email') }}" required autocomplete="username" class="w-full rounded-sm border border-white/25 bg-white px-10 py-3 text-secondary placeholder:text-text-main focus:border-brand-gold focus:ring-brand-gold/30" placeholder="00000 o usuario@correo.pe">
                    </div>
                </div>
                <div>
                    <label for="password" class="mb-2 block text-xs font-bold uppercase tracking-[0.15em] text-white/80">Contrasena</label>
                    <div class="relative">
                        <span class="material-icons-outlined pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-text-main">lock</span>
                        <input id="password" name="password" type="password" required autocomplete="current-password" class="w-full rounded-sm border border-white/25 bg-white px-10 py-3 text-secondary placeholder:text-text-main focus:border-brand-gold focus:ring-brand-gold/30" placeholder="••••••••">
                    </div>
                </div>
