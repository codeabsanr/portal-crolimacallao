    ...
    <div class="space-y-6">
        @for ($i = 0; $i < ($limit ?? 3); $i++)
            <article class="flex flex-col sm:flex-row gap-4 group cursor-pointer">
                <div
                    class="bg-primary-tint w-full sm:w-32 h-24 flex-shrink-0 rounded-sm flex items-center justify-center text-text-main">
                ...
                <div>
                    ...
                    <h3
                        class="text-xl font-bold text-text-main group-hover:text-primary transition-colors mb-2">
                        Título de noticia de ejemplo {{ $i + 1 }}
                    </h3>
                    <span class="text-sm text-text-main">Hace {{ $i + 1 }} días</span>
                </div>
            </article>
        @endfor
    </div>
